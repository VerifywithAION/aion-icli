...
if ($head -ne $expectedHead) {
  throw "Unexpected HEAD for release draft. Expected $expectedHead, got $head"
}
...
foreach ($r in $requiredText) {
  if (-not $draft.Contains($r)) {
    throw "Release draft missing required text: $r"
  }
}

foreach ($r in @("Target commit: 2fea528", "Asset: dist/aion-icli-public-install-package-v1.zip", "SHA256: 8B99C3C7161F2911212E7D57A4F3A3782700DBBCE404288D1E4AD6A671D7D746")) {
