...
$stage = ".\dist\aion-icli-public-install-package-v1"
...
$required = @($zip, $stage, $doc, $manifest, $report)
...
foreach ($p in $mustExistInStage) {
  $target = Join-Path $stage $p
  if (-not (Test-Path -LiteralPath $target)) {
    throw "Missing file in package stage: $p"
  }
}
...
foreach ($f in $forbidden) {
  ...
  if (Test-Path -LiteralPath $target) {
    throw "Forbidden path included in package stage: $f"
  }
}
